...
async def get_table_schemas(
    ...
    """
    現在のデータベーステーブルスキーマと関連メタデータを取得する。
    
    戻り値:
        UniversalResponse: テーブルスキーマのリスト、推定されたテーマ、テーブル数、テーブル名を含むオブジェクト。
    
    例外:
        HTTPException: テーブルが存在しない場合は404、データベースエラーまたは予期しないエラーの場合は500。
    """
    try:
        ...
        # 簡単なテーマ推測ロジック
        if any("employee" in name.lower() or "department" in name.lower() for name in table_names):
            theme = "社員管理"
        elif any("product" in name.lower() or "order" in name.lower() or "customer" in name.lower() for name in table_names):
            theme = "ECサイト"
        elif any("student" in name.lower() or "course" in name.lower() or "enrollment" in name.lower() for name in table_names):
            theme = "学校管理"
        elif any("book" in name.lower() or "author" in name.lower() or "library" in name.lower() for name in table_names):
            theme = "図書館"
        
        logger.info(f"Successfully retrieved {table_count} table schemas for theme: {theme}")
